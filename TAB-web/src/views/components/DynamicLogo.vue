...
// 准备logo数据
import { useWeatherPictureStore } from '@/stores'
const weatherPic = useWeatherPictureStore()
const blackPic = weatherPic.getBlackPic()
const whitePic = weatherPic.getWhitePic()

/** 存储由logos生成的logoImg对象 */
let blackLogoImgs: Record<string, LogoImg> = {}
let whiteLogoImgs: Record<string, LogoImg> = {}
...
/** canvas实体对象 */
const particleCanvas = ref<ParticleCanvas>()
...
for (let item in blackPic) {
  blackLogoImgs[item] = new LogoImg(blackPic[item], item)
}

for (let item in whitePic) {
